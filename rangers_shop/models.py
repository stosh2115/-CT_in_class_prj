from werkzeug.security import generate_password_hash #generates a unique hased password to store in the database for security
from flask_sqlalchemy import SQLAlchemy #this is our ORM
...
import uuid #makes a unique id for our data
...
def load_user(user_id):
    ...
    return User.query.get(user_id) #this is probablt the most basic query, just grabs the row associated with that primary key
                                    #similar to select * FROM user WHERE user_id = user_id



#create our User class/table. think of these as admin user
...
class Product(db.Model):
    ...
    def set_image(self, image, name):
        ...
        if not image: #aka the user did not give us an image
            image = get_image(name) #name is our argument replace the search parametet in our get_image() function
...
# because an order can have many Product but Product can be a part of many Orders 
...
#create our schem class (aka what are data will looki like when we pass it to the frontend)
#data also cannot be an ogject but rather a dictionary (json)
