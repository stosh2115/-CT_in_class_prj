...
def shop():
    ...
    #grab all of the products in our database cia query
    allprods = Product.query.all() #same as SELECT * FROM products
    ...
    shop_stats = {
        ...
        'sales': sum([order.order_total for order in allorders]) #looping through allorders list, order is each object and im grabbing the order_total 
    }


    return render_template('shop.html', shop=allprods, stats=shop_stats) #looking inside site_template folder for a file called shop.html to render


@site.route('/shop/create', methods=['GET', 'POST'])
def create():

    #instantaite our productfrom class
    ...
    if request.method == 'POST' and createform.validate_on_submit():
        ...
        flash("We were uable to process your request", category='warning')
